...
Metrics are heavy-tailed, so we use non-parametric tests: Kruskal--Wallis over all \(NB\), then Mann--Whitney U for each \(NB\ge2\) vs.\ \(NB=1\), with Holm correction.
...
\caption{Step length = 100. All \(NB\ge2\) are significantly better than \(NB=1\) for both models (Holm-corrected).}
\label{tab:stats}
\end{table}

\textbf{Interpretation.} The gain of \(NB\ge2\) over \(NB=1\) is significant for all metrics and both models. So the effect of \(NB\) is real, not noise. The table also shows that for Stochastic Mixture, \(NB=7\) is not always in the ``significant vs.\ \(NB=1\)'' set (e.g.\ for Categorical MMD only 2--6 are listed); that fits with the idea that the best \(NB\) depends on the metric and that very large \(NB\) does not always help.
